...
int test_id = 1056;
inline bool CNpc::RegisterRegion(float x, float z, int id)
{
	 if(m_sNid == test_id)
	 {
	  int x = 0;
...
void CNpc::SendMoveResult(float fX, float fY, float fZ, float fSpeed)
{
	...
	result << (fSpeed);
	g_pMain->Send(&result);
	RegisterRegion(fX, fZ, GetID() + NPC_BAND);
}
...
time_t CNpc::NpcMoving()
{
	...
	//SendMoveResult(m_fPrevX, m_fPrevY, m_fPrevZ, (float)(0.0f * ((double)m_sSpeed / 3000)));
	SendMoveResult(m_fPrevX, m_fPrevY, m_fPrevZ, (float)m_sSpeed / 1000);
	return m_sSpeed;	
}
...
time_t CNpc::NpcStanding()
{
	...
	if (GetType() == NPC_SPECIAL_GATE && GetMap()->isWarZone() && g_pMain->m_byBattleEvent == BATTLEZONE_OPEN)
	{
	...
	if (GetType() == NPC_SPECIAL_GATE && g_pMain->m_byBattleEvent == CLAN_BATTLE)
	{
		if(m_byGateOpen && UnixGateClose == 30)
		{
			m_byGateOpen = !m_byGateOpen;
...
bool CNpc::SetLive()
{
	...
	if (m_bFirstLive)	
	{
			...
			&& !m_bIsEventNpc)
		{
			...
			printf("Serverdaki Tum MOB ve NPC'ler Yuklendi - %d\n", (uint16) g_pMain->m_TotalNPC);
			color_c("", FOREGROUND_INTENSITY + FOREGROUND_GREEN);
	...
	RegisterRegion(GetX(), GetZ(), GetID() + NPC_BAND);
	m_byDeadType = 0;
...
bool CNpc::FindEnemy()
{
	...
	bool bIsNeutralZone = (GetZoneID() == ZONE_MORADON || GetZoneID() == ZONE_MORADON2
		|| GetZoneID() == ZONE_MORADON3 || GetZoneID() == ZONE_MORADON4
		|| GetZoneID() == ZONE_MORADON5 || GetZoneID() == ZONE_ARENA); 
...
bool CNpc::StepMove()
{
	...
	if(m_pPoint[m_iAniFrameCount].fXPos < 0 || m_pPoint[m_iAniFrameCount].fZPos < 0)
	{
		...
		RegisterRegion(m_fPrevX, m_fPrevZ, GetID() + NPC_BAND);
		return false;	
	}
	...
	if (m_sStepCount++ > 0)
	{
		...
		return RegisterRegion(GetX(), GetZ(), GetID() + NPC_BAND);
	}
...
void CNpc::GiveNpcHaveItem()
{
	...
	if (pItem != nullptr)
	{
		...
		for (int j = 0; j < 5; j++)
		{
			...
			if (iRandom < 1000)
				iRandom = iRandom*(5/2);
			else if (iRandom > 1000 && iRandom < 3000)
				iRandom = iRandom*2;
...
void CNpc::ChangeAbility(int iChangeType)
{
	...
	if (iChangeType == BATTLEZONE_OPEN || iChangeType == NATION_BATTLE || iChangeType == SNOW_BATTLE || iChangeType == CLAN_BATTLE)
	{
...
void CNpc::color_c(char* buff, WORD color)
{
...
void CNpc::Yaz(char* yazi, WORD color)
{
	char data[105];
