...
bool CMagicProcess::GrantType4Buff(_MAGIC_TABLE * pSkill, _MAGIC_TYPE4 *pType, Unit * pCaster, Unit *pTarget, bool bIsRecastingSavedMagic /*= false*/)
{
	...
	case BUFF_TYPE_ATTACK_DAMAGE_AMOUNT:						//Adjusted to Attack (Beserker) by Obede
		switch (pSkill->iNum)
			{
			case 115580:
				if (pTarget->isPlayer())
				{
					TO_USER(pTarget)->SetStatBuff2(STAT_STR, pType->bStr);
					TO_USER(pTarget)->SetStatBuff2(STAT_STA, pType->bSta);
					TO_USER(pTarget)->SetStatBuff2(STAT_DEX, pType->bDex);
					TO_USER(pTarget)->SetStatBuff2(STAT_INT, pType->bIntel);
					TO_USER(pTarget)->SetStatBuff2(STAT_CHA, pType->bCha);
					TO_USER(pTarget)->m_sACPercent += (pType->sACPct - 100);	
	...
	case BUFF_TYPE_FREEZE:
		...
		pTarget->m_bBlockMagic = true;                  //Block Magical Attack Damage (But Effect Work) by Obede
	...
	case BUFF_TYPE_TRIPLEAC_HALFSPEED:	// Wall of Iron || Divide Armor
		switch (pSkill->iNum)
		{
		case 115820:
			pTarget->m_sACPercent += (pType->sACPct - 100);
	...
	case BUFF_TYPE_KAUL_TRANSFORMATION:	// Transforms the target into a Kaul (a pig thing), preventing you from /town'ing or attacking, but increases defense.
		if (pTarget->isPlayer())
		{
			TO_USER(pTarget)->m_bIsKaul = true;
			pTarget->m_sACAmount += 500;
			pTarget->StateChangeServerDirect(3, pType->iNum);
		}
		break;

	case BUFF_TYPE_DEVIL_TRANSFORM:			// Devil Transform by Obede
...
bool CMagicProcess::RemoveType4Buff(uint8 byBuffType, Unit *pTarget, bool bRemoveSavedMagic /*= true*/, bool bRecastSavedMagic /*= false*/)
{
	...
	_MAGIC_TYPE4 * pType = g_pMain->m_Magictype4Array.GetData(itr->second.m_nSkillID);       //Missing remove Bufs - Fixed by Obede
	...
	case BUFF_TYPE_FISHING_FLASH:
		if (pTarget->isPlayer() && pType->sSpecialAmount == 1)
			TO_USER(pTarget)->m_FlashDcBonus = 0;
		if (pTarget->isPlayer() && pType->sSpecialAmount == 2)
			TO_USER(pTarget)->m_FlashExpBonus = 0;
		if (pTarget->isPlayer() && pType->sSpecialAmount == 3)
			TO_USER(pTarget)->m_FlashWarBonus = 0;
		break;
	...
	case BUFF_TYPE_ATTACK_DAMAGE_AMOUNT:						//Adjusted to Attak(Beserker)By Obede
		switch (pSkill->iNum)
			{
			case 115580:
				if (pTarget->isPlayer())
				{
					TO_USER(pTarget)->SetStatBuff2(STAT_STR, 0);
					TO_USER(pTarget)->SetStatBuff2(STAT_STA, 0);
					TO_USER(pTarget)->SetStatBuff2(STAT_DEX, 0);
					TO_USER(pTarget)->SetStatBuff2(STAT_INT, 0);
					TO_USER(pTarget)->SetStatBuff2(STAT_CHA, 0);
					TO_USER(pTarget)->m_sACPercent = 100;	
					if (pType->bAttack > 100)
						TO_USER(pTarget)->m_bAttackAmount -= (pType->bAttack - 100);
					else
	...
	case BUFF_TYPE_FREEZE:
		// Proportional to the target user's current HP.
		pTarget->m_bBlockMagic = false;               // Remove Blockes Magical Damage   -  By Obede
		pTarget->m_bSpeedAmount = 100;
		break;
	...
	case BUFF_TYPE_KAUL_TRANSFORMATION:	// Transforms the target into a Kaul (a pig thing), preventing you from /town'ing or attacking, but increases defense.
		if (pTarget->isPlayer())
		{
			TO_USER(pTarget)->m_bIsKaul = false;
			pTarget->m_sACAmount -= 500;
	...
	if (pTarget->isPlayer() && bRecastSavedMagic && TO_USER(pTarget)->isLockableScroll(pType->bBuffType))
	{
		TO_USER(pTarget)->SendUserStatusUpdate(USER_STATUS_POISON, USER_STATUS_CURE);
		TO_USER(pTarget)->RecastLockableScrolls(pType->bBuffType);
	}

	if (pTarget->isNPC() && bRecastSavedMagic && TO_USER(pTarget)->isLockableScroll(pType->bBuffType))
		TO_USER(pTarget)->RecastLockableScrolls(pType->bBuffType);
