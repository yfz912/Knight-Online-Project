...
void CUser::QuestV2PacketProcess(Packet & pkt)
{
	...
	case 7:
		...
		QuestV2MonsterDataRequest(nQuest);
		break;
...
void CUser::SaveEvent(uint16 sQuestID, uint8 bQuestState)
{
	...
	if (bQuestState == 1 && pQuestMonster != nullptr)
	{
		...
		for(int i = 0; i < QUEST_TOTAL; i++) // BURASI bende farklı , bendeki sistem biraz degisşik hm
		{
...
void CUser::QuestV3MainCount(uint16 sNpcID)
{
	for (int i = 0; i < QUEST_TOTAL; i++)
	{
		...
		QuestV3MonsterCount(uQuest->dQuestID, sNpcID);
	}
}

void CUser::QuestV3MonsterCount(uint16 uQuestID, uint16 sNpcID)
{
	_QUEST_MONSTER *pQuestMonster = g_pMain->m_QuestMonsterArray.GetData(uQuestID);
	...
	for (int Count = 0; Count < QUEST_MOB_GROUPS; Count++)
	{
		for (int i = 0; i < QUEST_MOBS_PER_GROUP; i++)
		{
			...
			result << uint8(2) << uint16(uQuestID) << uint8(Count + 1) << uint16(m_bKillCounts[Count]);
			Send(&result);
			...
			if (m_bKillCounts[Count] >= pQuestMonster->sCount[Count]) //Closing conditions
			{
				...
				Quest_Moster_Couner_Map::iterator itr = m_quest_moster_map.find(uQuestID);
				if(itr == m_quest_moster_map.end())
				{
					m_quest_moster_map.insert(std::make_pair(uQuestID,m_bKillCounts[Count]));
				}
				else
				{
...
void CUser::QuestV2MonsterDataRequest(uint16 uQuestID)
{
	Packet result(WIZ_QUEST, uint8(9));

	uint16 rQuest = uQuestID;
...
{
	uint8 bNewClasses[] = { ClassWarriorNovice, ClassRogueNovice, ClassMageNovice, ClassPriestNovice, ClassKurPorNovice };
	uint8 bOldClass = GetClassType() - 1; // convert base class 1,2,3,4,13 to 0,1,2,3,4 to align with bNewClasses

	// Make sure it's a beginner class.
	if (!isBeginner())
		return false;

	if (bOldClass == 12) // Valid Kurian to array 4
		bOldClass = 4;

	Packet result(WIZ_CLASS_CHANGE, uint8(PROMOTE_NOVICE));
