...
bool CGameServerDlg::Startup()
{
	...
	printf("########################################################################################################################");
	printf("########################################################################################################################");
	printf("#############################         EuroPvP V21xx KNIGHT ONLINE SERVER PROJECT        ##################################");
	...
	m_fpTradeUser = fopen(string_format("./Logs/TradeUser_%d_%d_%d.log",time.GetDay(),time.GetMonth(),time.GetYear()).c_str(), "a");
	if (m_fpTradeUser == nullptr)
	{
		printf("ERROR: Unable to open Trade user log file.\n");
		return false;
	}
	...
	m_fpShop = fopen(string_format("./Logs/NPCShop_%d_%d_%d.log",time.GetDay(),time.GetMonth(),time.GetYear()).c_str(), "a");
	if (m_fpShop == nullptr)
	{
		printf("ERROR: Unable to open NPCShop log file.\n");
		return false;
	}
...
void CGameServerDlg::GetTimeFromIni()
{
	...
	m_nGameMasterRHitDamageUser = ini.GetInt("SETTINGS","GAME_MASTER_R_HIT_DAMAGE_USER", 1000);
	...
	m_Grade2 = ini.GetInt("SETTINGS","GRADE2", 340000);
	m_Grade3 = ini.GetInt("SETTINGS","GRADE3", 50000);
	m_Grade4 = ini.GetInt("SETTINGS","GRADE4", 25000);

	m_MerchantStatus = ini.GetInt("STATUS","MERCHANT_STATUS", 1);
	m_TradeStatus = ini.GetInt("STATUS","TRADE_STATUS", 1);

	m_upgrade2 = ini.GetInt("UPGRADE_RATES","KARIVDIS_RATE", 10000);
	m_upgrade1 = ini.GetInt("UPGRADE_RATES","TRINA_RATE", 10000);
	...
	m_nEventMonumentNation[ZONE_RONARK_LAND] = 0;
...
uint32 CGameServerDlg::Timer_CheckGameEvents(void * lpParam)
{
	while (g_bRunning)
	{
		...
		g_pMain->CastleSiegeWarOpenTimer();		//CastleSiegeWar Times
		if (g_pMain->isWarOpen() && ((UNIXTIME - g_pMain->m_lastBlessTime) >= (5 * NATION_MONUMENT_REWARD_SECOND)))
			g_pMain->CheckNationMonumentRewards();
		else if (g_pMain->pTempleEvent.ActiveEvent == TEMPLE_EVENT_BORDER_DEFENCE_WAR && (UNIXTIME - g_pMain->m_lastBorderTime >= (10 * NATION_MONUMENT_REWARD_SECOND)))
			g_pMain->BorderMonumentBosses();
		else if (g_pMain->m_byBattleOpen && ((UNIXTIME - g_pMain->m_CastleAnnounceTime) >= CLASTLE_ANNOUCE_REWARD_SECOND))
		{
			std::string sCastleAnnounce;
			g_pMain->m_CastleAnnounceTime = UNIXTIME;
			sCastleAnnounce = string_format("CastleSiegeWar running, overcome all clans, Attack!!!");
			g_pMain->SendAnnouncement(sCastleAnnounce.c_str());
		}
		sleep(1 * SECOND);
	}
	return 0;
											 ...
											 uint8 nation, uint8 seekingPartyOptions, uint16 nEventRoom)
{
	...
	foreach (itr, sessMap)
	{
		...
		if (pUser->GetZoneID() == ZONE_MORADON || pUser->GetZoneID() == ZONE_MORADON2
			|| pUser->GetZoneID() == ZONE_MORADON3 || pUser->GetZoneID() == ZONE_MORADON4 
			|| pUser->GetZoneID() == ZONE_MORADON5 || pUser->GetNation() == nation)
		{
...
void CGameServerDlg::UpdateGameTime()
{
	...
	// Juraid Deval Kill Code By EvaneScence
	if (m_sSec != now.GetSecond())
	{
		if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_JURAD_MOUNTAIN && pTempleEvent.isDevaControl)
		{
			m_DevakillSecond++;

			if (pTempleEvent.isDevaFlag && pTempleEvent.isAttackable && pTempleEvent.isActive)
			{
				pTempleEvent.isAttackable = false;
...
void CGameServerDlg::GetRegionUserIn(C3DMap *pMap, uint16 region_x, uint16 region_z, Packet & pkt, uint16 & t_count, uint16 nEventRoom)
{
	...
	foreach (itr, pRegion->m_RegionUserArray)
	{
		...
		if (nEventRoom > 0 && !pUser->isGM() && nEventRoom != pUser->GetEventRoom()) // gm ise tüm çarları gör test kontrol et
			continue;
...
void CGameServerDlg::BattleZoneOpenTimer()
{
	...
	std::string sWarAnnounce;
	std::string sWarAnnounce2;
	std::string sWarAnnounce3;
	sWarAnnounce = string_format("10 Minutes Before War Commences!");
	sWarAnnounce2 = string_format("5 Minutes before War Commences!");
	sWarAnnounce3 = string_format("1 Minute Before War Commences!");

	if (!isWarOpen() && (nMin == 50 || nMin == 55 || nMin == 59))
	{
		std::list<std::string> vargs = StrSplit(m_sBattleZoneOpenDays, ",");
		uint8 nDaySize = vargs.size();
		if (nDaySize > 0)
		{
			...
			for (int i = 0; i < nDaySize; i++)
			{
				...
				if (nDay == nWeekDay)
				{
					for (int x = 0; x < WAR_TIME_COUNT; x++)
					{
						if ((m_nBattleZoneOpenHourStart[x] - 1) == nHour && nMin == 50)
						{
							if ((m_nBattleZoneOpenHourStart[x] - 1) == nHour && nSec == 00)
							{
								LunarGoldShells();
								SendAnnouncement(sWarAnnounce.c_str());
								SendNotice(sWarAnnounce.c_str());
							}
						}
						else if ((m_nBattleZoneOpenHourStart[x] - 1) == nHour && nMin == 55)
						{
							if ((m_nBattleZoneOpenHourStart[x] - 1) == nHour && nSec == 00)
							{
								SendAnnouncement(sWarAnnounce2.c_str());
								SendNotice(sWarAnnounce2.c_str());
							}
						}
						else if ((m_nBattleZoneOpenHourStart[x] - 1) == nHour && nMin == 59)
						{
							if ((m_nBattleZoneOpenHourStart[x] - 1) == nHour && nSec == 00)
							{
								SendAnnouncement(sWarAnnounce3.c_str());
								SendNotice(sWarAnnounce3.c_str());
							}
						}
					}
				}
	...
	else if (m_byBattleOpen == NATION_BATTLE)	
	{
		...
		if (m_bVictory == 0)
		{
			...
			if (m_sBattleTimeDelay >= (nBattleZone == ZONE_BATTLE6 ? (m_byBattleTime / 48) : (m_byBattleTime / 24)))
			{
				m_sBattleTimeDelay = 0;
				Announcement(DECLARE_BATTLE_ZONE_STATUS);
			}
...
void CGameServerDlg::BattleZoneResult(uint8 nation, CUser* pUser)
{
	m_bVictory = nation;
	...
	// Achieve War Imput system by Obede
	if (!AlreadySendAchieve)
	{
		SessionMap sessMap = g_pMain->m_socketMgr.GetActiveSessionMap();
		foreach (itr, sessMap)
		{
			CUser * wUser = TO_USER(itr->second);
			if (wUser == nullptr 
				|| !wUser->isInGame()
				|| wUser->GetNation() != m_bVictory)
				continue;
    
			wUser->AchieveMainCount(0, 4, 0, 0);
		}
		AlreadySendAchieve = true;
	}
...
void CGameServerDlg::BattleWinnerResult(BattleWinnerTypes winnertype)
{
	...
	else if (winnertype == BATTLE_WINNER_MONUMENT)
	{
		...
		if (winner_nation == 0 && (nBattleZone == ZONE_BATTLE4))
		{
			BattleWinnerResult(BATTLE_WINNER_KILL);
			return;
		}
	...
	else if (winnertype == BATTLE_WINNER_KILL)	
	{
		...
		if (winner_nation == 0 && (nBattleZone == ZONE_BATTLE6)) 
		{
			BattleWinnerResult(BATTLE_WINNER_MONUMENT);
			return;
		}
...
void CGameServerDlg::CastleSiegeWarOpenTimer()
{
	int nDay = g_localTime.tm_wday;
	int nHour = g_localTime.tm_hour;
	int nMin = g_localTime.tm_min;
	_KNIGHTS_SIEGE_WARFARE* pInfo = nullptr;

	pInfo = m_KnightsSiegeWarfareArray.GetData(1);
	if(!pInfo)
		return;

	if (m_byBattleOpen == CLAN_BATTLE && m_byBattleSiegeWarTown == true)					//Edited by Obede(Fix Crystal friend in town)
	{
		if (UNIXTIME - g_pMain->m_lastCrystalKilled == WAR_MONUMENT_REWARD_SECOND)
		{
	...
	if( !m_byBattleSiegeWarOpen)
	{
		if(nDay  == pInfo->byWarDay && nHour ==  pInfo->byWarTime && nMin == pInfo->byWarMinute)
		{
	...
	if (m_byBattleOpen == CLAN_BATTLE)
	{
		...
		else if( m_sBattleTimeDelay == ( 70 * MINUTE ) && !m_byBattleSiegeWarMomument)
		{
			Announcement(IDS_SIEGE_WAR_END);
			m_sBattleTimeDelay = 0;
			m_byBattleOpen = NO_BATTLE;
...
void CGameServerDlg::TempleEventTimer()
{
	...
	if (pTempleEvent.ActiveEvent == -1)
	{
		...
		for (int i = 0; i < UTC_EVENT_COUNT; i++)
		{
	...
	else if (pTempleEvent.ActiveEvent != -1)
	{
		if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_BORDER_DEFENCE_WAR)
		{
			...
			for (int i = 0; i < BORDER_DEFENSE_WAR_EVENT_COUNT; i++)
			{
				...
				else if (nHour == m_nBorderDefenseWarTime[i] && nMinute == 10 && !pTempleEvent.isAttackable)   
				{
		...
		if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_JURAD_MOUNTAIN)
		{
			for(int i = 0; i < JURAD_MOUNTAIN_EVENT_COUNT; i++)
			{
				...
				else if (nHour == m_nJuraidTime[i] && nMinute == 10 && !pTempleEvent.isAttackable)   
				{
					pTempleEvent.isAttackable = true;
					break;
				}
...
void CGameServerDlg::JuraidTempleEventStart()
{
	...
	if (JuraidAnnounce == 0)
	{
		Announcement(IDS_JURAD_EVENT_ANNOUNCE);
		JuraidAnnounce++;
	}
	else
...
void CGameServerDlg::TempleEventTeleportUsers()
{
	...
	foreach_stlmap (itr, m_TempleEventUserArray)
	{
		...
		else if (pTempleEvent.ZoneID == ZONE_UNDER_THE_CASTLE)
		{
			pUser->ZoneChange(pTempleEvent.ZoneID,0.0f,0.0f);
			TerminitionTime(ZONE_BORDER_DEFENSE_WAR, 3601);
		}
	}
}
...
void CGameServerDlg::TempleEventFinish(uint16 m_sEventRoom, uint8 m_bNation)
{
	foreach_stlmap (itr, m_TempleEventUserArray)
	{
		...
		if (pRankInfo)
		{
			...
			if (pRankInfo->m_bZone == ZONE_UNDER_THE_CASTLE)
			{
				pUser->GiveItem(GREEN_TREASURE_CHEST);				
			}
			if (pRankInfo->m_bZone == ZONE_JURAD_MOUNTAIN)
			{
				pUser->JuraidSpot = 0;
				if (g_pMain->pTempleEvent.ElmoDeathCount[pUser->GetEventRoom()] == pTempleEvent.KarusDeathCount[pUser->GetEventRoom()] && g_pMain->pTempleEvent.KarusDeathCount[pUser->GetEventRoom()] == pTempleEvent.ElmoDeathCount[pUser->GetEventRoom()])
				{
					...
					pUser->AchieveMainCount(0, 6, 0, 0);		// Valid Juraid Winners Count
				}
				else if (pTempleEvent.ElmoDeathCount[pUser->GetEventRoom()] > pTempleEvent.KarusDeathCount[pUser->GetEventRoom()] && pUser->GetNation() == KARUS) 
				{
					if (pUser->GetLevel() >= 70 && pUser->GetLevel() <= 72)
			...
			else if (pRankInfo->m_bZone == ZONE_CHAOS_DUNGEON)
			{
				...
				nChangeExp = pUser->m_bPremiumInUse != 0 ? nPremiumGainedExp : nGainedExp;
				...
				// Valid Chaos Place Count
				if (nUserRank == 1)
					pUser->AchieveMainCount(0, 7, 0, 0);
				else if (nUserRank == 2)
					pUser->AchieveMainCount(0, 8, 0, 0);
				else if (nUserRank == 3)
					pUser->AchieveMainCount(0, 9, 0, 0);
			}
...
void CGameServerDlg::Announcement(uint16 type, int nation, int chat_type, CUser* pExceptUser, CNpc *pExpectNpc)
{
	...
	case IDS_SIEGE_WAR_VICTORY:
		...
		break;	
	case IDS_JURAD_EVENT_ANNOUNCE:
		GetServerResource(IDS_JURAD_EVENT_ANNOUNCE, &chatstr);
		break;
	case IDS_JURAD_EVENT_ANNOUNCE2:
		GetServerResource(IDS_JURAD_EVENT_ANNOUNCE2, &chatstr);
		break;
	case IDS_JURAD_EVENT_ANNOUNCE3:
		GetServerResource(IDS_JURAD_EVENT_ANNOUNCE3, &chatstr);
		break;
...
void CGameServerDlg::NoahKnights(Packet * pkt)
{
	for (int i = 0; i < MAX_USER; i++)
	{
		CUser * pUser = GetUserPtr(i);

		if (pUser == nullptr || pUser->GetLevel() > 50)
			continue;
...
uint16 CGameServerDlg::GetKnightsAllMembers(uint16 sClanID, Packet & result, uint16 & pktSize, bool bClanLeader)
{
	...
	foreach_array (i, pKnights->m_arKnightsUser)
	{
		...
		if( pCurrent == nullptr )
		{
			//if(p->m_bLevel == 0 || p->m_sFame == 0 || p->m_sClass == 0)   Edited by Obede
			g_DBAgent.GetClanUserData(sClanID,p);
			result << p->strUserName << uint16(p->m_sFame) << p->m_bLevel << p->m_sClass << uint8(g_pMain->GetUserPtr(p->strUserName, TYPE_CHARACTER) == nullptr ? 0 : 1) << p->strUserMemo << uint32(UNIXTIME - p->m_lastLogin);
		}
		else
			result << pCurrent->GetName() << uint16(pCurrent->GetFame()) << pCurrent->GetLevel() << pCurrent->m_sClass << uint8(g_pMain->GetUserPtr(p->strUserName, TYPE_CHARACTER) == nullptr ? 0 : 1) << pCurrent->m_strMemo << uint32(UNIXTIME - p->m_lastLogin);
...
void CGameServerDlg::BorderMonumentBosses()
{
	std::vector<uint16> deleted;
	std::string sMonuKarus;
	std::string sMonuHuman;

	sMonuKarus = string_format("Karus has blessed the [Border War Monument].Bosses appears on Karus Base!");
	sMonuHuman = string_format("ElMorad has blessed the [Border War Monument].Bosses appears on Human Base!");
	...
	foreach_stlmap (itr, m_NationMonumentInformationArray)
	{
		...
		{		
			...
			if(pTarget == nullptr || pTarget->isNPC() || pTarget->GetNation() != pNpc->GetNation())
				continue;

			if (pTarget->GetNation() == pNpc->GetNation() && pTarget->GetZoneID() == ZONE_BORDER_DEFENSE_WAR)
				distributed_member.push_back(pTarget);
		}
		...
		if (pNpc->GetNation() == KARUS)
		{
			g_pMain->m_lastBorderTime = UNIXTIME;
			g_pMain->SendAnnouncement(sMonuKarus.c_str());
			SpawnEventNpc(1321,true,pNpc->GetZoneID(),50,0,131,10,20,300);
			SpawnEventNpc(1452,true,pNpc->GetZoneID(),55,0,131,5,10,300);
		}
		else
		{
			g_pMain->m_lastBorderTime = UNIXTIME;
			g_pMain->SendAnnouncement(sMonuHuman.c_str());
			SpawnEventNpc(1321,true,pNpc->GetZoneID(),207,0,131,10,20,300);
			SpawnEventNpc(1452,true,pNpc->GetZoneID(),202,0,131,5,10,300);
		}
	}
...
{
	fwrite(logMessage.c_str(), logMessage.length(), 1, m_fpTradeUser);
	fflush(m_fpTradeUser);
}
...
void CGameServerDlg::WriteShopLogFile(string & logMessage)
{
	fwrite(logMessage.c_str(), logMessage.length(), 1, m_fpShop);
	fflush(m_fpShop);
}
...
{
	if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_JURAD_MOUNTAIN && sNewZone == ZONE_JURAD_MOUNTAIN)
	{
		Packet aresult(WIZ_SELECT_MSG);
		aresult << uint16(0x00) << uint8(0x07) << uint64(0x00) << uint8(0x06) << uint16(0x00) << uint8(0x00) << uint8(0x00) << uint8(0x09) << Time << uint16(0x00);
		
		Packet bresult(WIZ_BIFROST, uint8(0x05));
		bresult << Time;
	...
	else if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_BORDER_DEFENCE_WAR && sNewZone == ZONE_BORDER_DEFENSE_WAR)
	{
	...
	else if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_CHAOS && sNewZone == ZONE_CHAOS_DUNGEON)
	{
		Packet aresult(WIZ_SELECT_MSG);
		aresult << uint16(0)   << uint8(7)    
				<< uint32(0)   << uint32(0) 
...
void CGameServerDlg::LunarGoldShells(){
    SessionMap sessMap = g_pMain->m_socketMgr.GetActiveSessionMap();
    foreach (itr, sessMap)
    {
...
void CGameServerDlg::GetMineItem()
{
	CIni ini(CONF_MINE_LIST);

	m_Drop1 = ini.GetInt("MINING","Drop1", 0);
	m_Drop2 = ini.GetInt("MINING","Drop2", 0);
	m_Drop3 = ini.GetInt("MINING","Drop3", 0);
	m_Drop4 = ini.GetInt("MINING","Drop4", 0);
	m_Drop5 = ini.GetInt("MINING","Drop5", 0);
	m_Drop6 = ini.GetInt("MINING","Drop6", 0);
	m_Drop7 = ini.GetInt("MINING","Drop7", 0);
	m_Drop8 = ini.GetInt("MINING","Drop8", 0);
	m_Drop9 = ini.GetInt("MINING","Drop9", 0);
	m_Drop10 = ini.GetInt("MINING","Drop10", 0);
	m_Drop11 = ini.GetInt("MINING","Drop11", 0);
	m_Drop12 = ini.GetInt("MINING","Drop12", 0);
	m_Drop13 = ini.GetInt("MINING","Drop13", 0);
	m_Drop14 = ini.GetInt("MINING","Drop14", 0);
	m_Drop15 = ini.GetInt("MINING","Drop15", 0);
	m_rainworm = ini.GetInt("MINING","Rainworm", 0);

	m_gDrop1 = ini.GetInt("GOLD_MINING","Drop1", 0);
	m_gDrop2 = ini.GetInt("GOLD_MINING","Drop2", 0);
	m_gDrop3 = ini.GetInt("GOLD_MINING","Drop3", 0);
	m_gDrop4 = ini.GetInt("GOLD_MINING","Drop4", 0);
	m_gDrop5 = ini.GetInt("GOLD_MINING","Drop5", 0);
	m_gDrop6 = ini.GetInt("GOLD_MINING","Drop6", 0);
	m_gDrop7 = ini.GetInt("GOLD_MINING","Drop7", 0);
	m_gDrop8 = ini.GetInt("GOLD_MINING","Drop8", 0);
	m_gDrop9 = ini.GetInt("GOLD_MINING","Drop9", 0);
	m_gDrop10 = ini.GetInt("GOLD_MINING","Drop10", 0);
	m_gDrop11 = ini.GetInt("GOLD_MINING","Drop11", 0);
	m_gDrop12 = ini.GetInt("GOLD_MINING","Drop12", 0);
	m_gDrop13 = ini.GetInt("GOLD_MINING","Drop13", 0);
	m_gDrop14 = ini.GetInt("GOLD_MINING","Drop14", 0);
	m_gDrop15 = ini.GetInt("GOLD_MINING","Drop15", 0);
	m_grainworm = ini.GetInt("GOLD_MINING","Rainworm", 0);
}

void CGameServerDlg::GetFishItem()
{
	CIni ini(CONF_FISHING_LIST);

	f_Drop1 = ini.GetInt("FISHING","FISHING", 0);
	f_Drop2 = ini.GetInt("FISHING","FISHING_1", 0);
	f_Drop3 = ini.GetInt("FISHING","FISHING_2", 0);
	f_Drop4 = ini.GetInt("FISHING","FISHING_3", 0);
	f_Drop5 = ini.GetInt("FISHING","FISHING_4", 0);
	f_Drop6 = ini.GetInt("FISHING","FISHING_5", 0);
	f_Drop7 = ini.GetInt("FISHING","FISHING_6", 0);
	f_Drop8 = ini.GetInt("FISHING","FISHING_7", 0);
	f_Drop9 = ini.GetInt("FISHING","FISHING_8", 0);
	f_Drop10 = ini.GetInt("FISHING","FISHING_9", 0);
	f_Drop11 = ini.GetInt("FISHING","FISHING_10", 0);
	f_Drop12 = ini.GetInt("FISHING","FISHING_11", 0);

	f_gDrop1 = ini.GetInt("GOLD_FISHING","FISHING", 0);
	f_gDrop2 = ini.GetInt("GOLD_FISHING","FISHING_1", 0);
	f_gDrop3 = ini.GetInt("GOLD_FISHING","FISHING_2", 0);
	f_gDrop4 = ini.GetInt("GOLD_FISHING","FISHING_3", 0);
	f_gDrop5 = ini.GetInt("GOLD_FISHING","FISHING_4", 0);
	f_gDrop6 = ini.GetInt("GOLD_FISHING","FISHING_5", 0);
	f_gDrop7 = ini.GetInt("GOLD_FISHING","FISHING_6", 0);
	f_gDrop8 = ini.GetInt("GOLD_FISHING","FISHING_7", 0);
	f_gDrop9 = ini.GetInt("GOLD_FISHING","FISHING_8", 0);
	f_gDrop10 = ini.GetInt("GOLD_FISHING","FISHING_9", 0);
	f_gDrop11 = ini.GetInt("GOLD_FISHING","FISHING_10", 0);
	f_gDrop12 = ini.GetInt("GOLD_FISHING","FISHING_11", 0);
}

void CGameServerDlg::GetEventTime()
{
	CIni ini(CONF_EVENT_TIMER);
	...
	for (int i = 0; i < BIFROST_EVENT_COUNT; i++)
		m_nBifrostTime[i] = ini.GetInt("BIFROST",string_format("START_TIME%d",i+1).c_str(), 99);

	for (int i = 0; i < BORDER_DEFENSE_WAR_EVENT_COUNT; i++)
		m_nBorderDefenseWarTime[i] = ini.GetInt("BDW",string_format("START_TIME%d",i+1).c_str(), 99);

	for (int i = 0; i < CHAOS_EVENT_COUNT; i++)
		m_nChaosTime[i] = ini.GetInt("CHAOS",string_format("START_TIME%d",i+1).c_str(), 99);

	for (int i = 0; i < JURAD_MOUNTAIN_EVENT_COUNT; i++)
		m_nJuraidTime[i] = ini.GetInt("JURAID",string_format("START_TIME%d",i+1).c_str(), 99);
	
	for (int i = 0; i < UTC_EVENT_COUNT; i++)
		m_nUtcTime[i] = ini.GetInt("UTC",string_format("START_TIME%d",i+1).c_str(), 99);
}
