...
{
    if (g_pMain->m_MerchantStatus == 0)
	return ;

	uint8 opcode = pkt.read<uint8>();

	if (isTrading() || isStoreOpen() || isDead())
		CancelMerchant();
...
void CUser::MerchantOpen()
{
	...
	else if (GetZoneID() > ZONE_MORADON5 || GetZoneID() <= ZONE_ELMORAD3 || isMining())
