...
void CUser::PartyRequest(int memberid, bool bCreate)
{
	...
	if ((GetNation() != pUser->GetNation() && (GetZoneID() < ZONE_MORADON || GetZoneID() > ZONE_MORADON5 )
...
void CUser::PartyBBSRegister(Packet & pkt)
{
	...
	foreach (itr, sessMap)
	{
		...
		if(pUser->m_bEventRoom != m_bEventRoom)
			continue;
...
void CUser::SendPartyBBSNeeded(uint16 page_index, uint8 bType)
{
	...
	foreach (itr, sessMap)
	{
		...
		if (GetZoneID() != pUser->GetZoneID()
			...
			&& (GetZoneID() != ZONE_MORADON || GetZoneID() != ZONE_MORADON2
