...
bool C3DMap::Initialize(_ZONE_INFO *pZone)
{
	...
	m_smdFile = SMDFile::Load(pZone->m_MapName, true);
...
bool C3DMap::CheckEvent(float x, float z, CUser* pUser)
{
	...
	if ( g_pMain->m_byBattleOpen == NATION_BATTLE && 
		(pUser->GetZoneID() != ZONE_KARUS_ESLANT || pUser->GetZoneID() != ZONE_KARUS_ESLANT2
		|| pUser->GetZoneID() != ZONE_KARUS_ESLANT3) && 
		(pUser->GetZoneID() != ZONE_ELMORAD_ESLANT || pUser->GetZoneID() != ZONE_ELMORAD_ESLANT2
		|| pUser->GetZoneID() != ZONE_ELMORAD_ESLANT3))
