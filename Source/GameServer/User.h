...
typedef std::map<uint16, uint8>				Quest_Moster_Couner_Map;

typedef CSTLMap <_NATION_DATA>				NationTransferArray;

// Time (in seconds) between each save request (3 min).
#define PLAYER_SAVE_INTERVAL      (2 * 60)
// Update Achieve time
#define ACHIEVE_TIME_UPDATE		(1 * MINUTE)
// time (in seconds) to verifi the User items 
#define PLAYER_CONF_INTERVAL		(5 * 60)
// Time (in seconds) between each reward online (1 hour).
#define PLAYER_ONLINE_REWARD      (1 * HOUR)
// Time (in seconds) to stay in Zone Stone (25 min).
#define TIME_MONSTER_STONE      (25 * 60)
...
// Time (in seconds) for Crystal monument
#define WAR_MONUMENT_REWARD_SECOND  11
// time for Castle Announcement
#define CLASTLE_ANNOUCE_REWARD_SECOND  (10 * 60)
// time genie updated
#define PLAYER_GENIE_INTERVAL	(1 * MINUTE)
// Time (in seconds) for training exp change
#define PLAYER_TRAINING_INTERVAL 10
// Time for Stamina Exchange KURIAN
#define PLAYER_STAMINA_INTERVAL	5
...
enum ClassType
{
	...
	ClassKurPor				= 13,
	ClassKurPorNovice		= 14,
	ClassKurPorMaster		= 15
};
...
public:
	...
	_ACHIEVE_DATA m_sAchieveDataArray[ACHIEVE_TOTAL];
	_ITEM_DATA m_sItemArray[INVENTORY_TOTAL];
	_ITEM_DATA m_sWarehouseArray[WAREHOUSE_MAX];
	_ITEM_DATA m_sVIPhouseArray[VIP_HAVE_MAX];
	_ITEM_DATA m_sItemArrayOrder[SLOT_MAX+HAVE_MAX];
	_PREMIUM_DATA m_sPremiumArray[PREMIUM_TOTAL];
	_QUEST_DATA m_sQuestArray[QUEST_TOTAL];

	uint8	m_bLogout;
	uint32	m_dwTime;
	time_t	m_lastSaveTime;
	time_t	m_lastUpdatedAchieveTime;
	time_t	m_LastTraining;
	time_t	m_LastConferedTime;
	time_t	m_lastBonusTime;
	time_t	m_TimeOnline;
	time_t	m_TimeMonsterStone;
	...
	// Achieve I
	uint32	m_AchieveTimeIn;	// Load from DB
	uint32	m_AchieveTimeOut;	// Update to DB
	time_t	m_tAchieveGameStartTime;
	uint32	MonDefeatCount;
	...
	uint8	c_AchieveNP;
	uint8	c_AchieveXP;
	time_t	aChallengeTime;
	uint16	ActiveAchieveChallenge;
	uint16	aCount;
	uint32	aUserContrib;
	uint32	aKnightContrib;

	uint8	m_bAccountStatus;
	uint8	m_bPremiumInUse;
	uint8	m_bPremiumCount;
	uint8	m_bPremiumExCount;
	...
	uint32	m_TrainingExp;
	uint16	m_TrainingTime;
	...
	int8	m_bStatBuffs2[STAT_COUNT];
	int8	m_bStatBuffs3[STAT_COUNT];
	int8	m_bRebStatBuffs[STAT_COUNT];
	...
	uint64 nExpRebirth[6];

	NationTransferArray		m_NationTransferArray;

	void UpdateStatusPacket();

public:
	INLINE uint16 GetGenieTime(){ return m_GenieTime; }
	INLINE bool isGenieActive() {return m_bGenieStatus;} // Genie 
	INLINE uint16 GetCoverTitle() { return iCoverTitle; } // Achieve Title 
	INLINE uint16 GetSkillTitle() {return iSkillTitle;} // Achieve Skille
	INLINE uint32 GetReturnStatus() { return isReturn; } // R SYMBOL
	...
	INLINE bool isKurPor() { return JobGroupCheck(ClassKurPor); }

	INLINE bool isBeginner() 
	{
		uint16 sClass = GetClassType();
		return (sClass == ClassWarrior || sClass == ClassRogue|| sClass == ClassMage || sClass == ClassPriest || sClass == ClassKurPor);
	}
	...
	INLINE bool isBeginnerKurPor()  { return GetClassType() == ClassKurPor; }

	INLINE bool isNovice() 
	{
		uint16 sClass = GetClassType();
		return (sClass == ClassWarriorNovice || sClass == ClassRogueNovice
			|| sClass == ClassMageNovice || sClass == ClassPriestNovice || sClass == ClassKurPorNovice); 
	}
	...
	INLINE bool isNoviceKurPor()  { return GetClassType() == ClassKurPorNovice; }

	INLINE bool isMastered() 
	{
		uint16 sClass = GetClassType();
		return (sClass == ClassWarriorMaster || sClass == ClassRogueMaster 
			|| sClass == ClassMageMaster || sClass == ClassPriestMaster || sClass == ClassKurPorMaster); 
	}
	...
	INLINE bool isMasteredKurPor()  { return GetClassType() == ClassKurPorMaster; }
	...
	INLINE bool isInEventZone() {  return GetZoneID() == ZONE_BORDER_DEFENSE_WAR || GetZoneID() == ZONE_JURAD_MOUNTAIN || GetZoneID() == ZONE_BIFROST; }
	...
	INLINE bool isLockableScroll(uint8 buffType) { return (buffType == BUFF_TYPE_HP_MP || buffType == BUFF_TYPE_AC || buffType == BUFF_TYPE_FISHING_FLASH || buffType == BUFF_TYPE_DAMAGE || buffType == BUFF_TYPE_SPEED || buffType == BUFF_TYPE_STATS); }
	...
	*/
	INLINE ClassType GetBaseClassType()
	{
		...
		{
			ClassWarrior, ClassRogue,	// job changed / mastered
			ClassMage, ClassPriest,		// job changed / mastered
			ClassKurPor,				// job changed / mastered
			...
			ClassKurPor, ClassKurPor	// job changed / mastered
		}; 
	...
	*/
	...
	virtual int32 GetMaxExp() { return *(uint32 *)&m_iMaxExp; }
	virtual int32 GetExp() { return *(uint32 *)&m_iExp; }
	...
	INLINE uint8 GetStatBuff2(StatType type)
	{
		ASSERT(type < STAT_COUNT);
		return m_bStatBuffs2[type];
	}

	INLINE void SetStatBuff2(StatType type, uint8 val)
	{
		ASSERT(type < STAT_COUNT);
		m_bStatBuffs2[type] = val;
	}

	INLINE uint8 GetStatBuff3(StatType type)
	{
		ASSERT(type < STAT_COUNT);
		return m_bStatBuffs3[type];
	}

	INLINE void SetStatBuff3(StatType type, uint8 val)
	{
		ASSERT(type < STAT_COUNT);
		m_bStatBuffs3[type] = val;
	}
	...
	INLINE _ACHIEVE_DATA * GetAchieve(uint16 ID) 
	{
		ASSERT(ID < ACHIEVE_TOTAL);
		return &m_sAchieveDataArray[ID]; 
	}

	INLINE _PREMIUM_DATA * GetPremiumPtr(uint8 pos)
	{
		ASSERT(pos < PREMIUM_TOTAL);
		return &m_sPremiumArray[pos];
	}

	INLINE _QUEST_DATA * GetQuestID(uint8 Slot)
	{
		ASSERT(Slot < QUEST_TOTAL);
		return &m_sQuestArray[Slot];
	}
	...
	void UpdateGenieTime(Packet & pkt);
	...
	// Achieve
	void Achieve(Packet & pkt);
	void HandleAchieveGetItem(Packet & pkt);
	void HandleAchieveInfo(Packet & pkt);
	void HandleAchieveShow(Packet & pkt);
	void HandleChallengeFailed(uint8 bType);
	void HandleChallengeStart(Packet & pkt);
	void HandleChallengeCancel(Packet & pkt);
	void HandleAchieveCoverTitle(Packet & pkt);	
	void HandleAchieveSkillTitle(Packet & pkt);
	void HandleAchieveCoverTitleReset(Packet & pkt);
	void HandleAchieveSkillTitleReset(Packet & pkt);
	void UpdateAchieveStatus(uint32 SkillID);
	void GiveAchieve(uint16 Title);
	void ResetAchieve();
	void AchieveUpdateKills(uint16 sNpcID);
	void AchieveMainCount(uint16 nNpcID, uint16 wType, uint16 cQuestNum, uint16 gKind, bool bKilled = false);
	void ChallengeMonsterCountAdd(uint16 aNpcID);
	bool aIsPlayer(uint16 ID);
	void StartChallengeStatus(uint16 ID);
	void UpdateChallengeStatus(uint16 ID, uint8 bType);
	void AchieveMonsterGroup(uint16 mGroup);
	void UserControl(uint16 MainID, uint16 wType, bool wKilled = false);
	void NpcKillControl(uint16 MainID, uint16 mNpcID, bool mKilled = false);
	void QuestControl(uint16 MainID, uint16 cQuestNum);
	void TitlesControl(uint16 MainID, uint16 gKind);

	// Pet Handlers
	void PetHandle(Packet & pkt);
	...
	void ChatroomSearch(Packet& pkt);
	void ChatroomUpdate(Packet & pkt);
	void SendChatRoom(Packet & result);	

	void AddUser(CUser * pUser);
	void ExpSealSystem(Packet & pkt);
	void ExpSealOn(Packet & pkt);
	void ExpSealOff(Packet & pkt);
	...
	bool CheckClass(short class1, short class2 = -1, short class3 = -1, short class4 = -1, short class5 = -1);
	...
	int GetEmptySlot();
	int GetCountEmptySlot();
	void SendAllKnightsID();
	void SendStackChange(uint32 nItemID, uint32 nCount /* needs to be 4 bytes, not a bug */, uint16 sDurability, uint8 bPos, bool bNewItem = false, uint32 ExpirationTime = 0);
	void SetTrainingMode();// Shortcuts for lazy people
	void StaminaTimeChange();
	void Type4Duration();
	void HPTimeChange();
	void HPTimeChangeType3();

	void LoadSpecialItem(Packet & pkt, uint32 rSealSerial);
		//UserInfo
	void RemoveRegionChat();
	void HandleUserInfo(Packet & pkt);
	void HandleUserInfoNick(Packet & pkt);
	void HandleUserInfoShow(Packet & pkt);
	void HandleUserInfoDetail(Packet & pkt);
	void HandleUnknow5(Packet& pkt);
	...
	void CharacterLocationSend();
	void CharacterLocationRecv(Packet & pkt);
	...
	void RecvPremium(Packet & pkt);
	void SendPremiumExpire();
	void Check_Expire();
	...
	// JACPOT SYSTEM
	void ExpEvent(int64 exp);
	void GoldEvent(uint32 gold);
	...
	COMMAND_HANDLER(HandlePacketCommand);
	...
	void RecvNationTransfer(Packet & pkt);
	void FindClanChar_Read(std::string strCharID, uint8 & error, uint16 & nNum, _NATION_DATA* & m_NationTransferArray);
	void SendNationTransfer();
	void RecvGenderChange(Packet & pkt);
	...
	void Listopen(Packet & pkt);
	void SealedCharacterInventory(Packet & pkt);
	void CharacterUnsealProcess(Packet & pkt);
	...
	void UpdateStatusCSW();
	void SetMaxSp();
	void StaminaChange(int amount);
	bool isMagicEffected(CUser* pCaster, uint32 nSkillID); // SucessRate by Obede
	void RecvOpenDeletedItems(Packet & pkt);
	void SendNotice();
	void TopSendNotice();
	void ClanKnight();
	...
	void ItemsDupeDelete();
	void IlegalItemsBanned();
	...
	void SaveAchieveEvent(uint16 sAQuestID, uint8 bAQuestState);
	void DeleteEvent(uint16 sQuestID);
	bool CheckExistEvent(uint16 sQuestID, uint8 bQuestState);
	bool CheckExistAquieve(uint16 sAQuestID, uint8 bAQuestState);

	///// TEST /////
	void QuestV3MainCount(uint16 sNpcID);
	void QuestV3MonsterCount(uint16 uQuestID, uint16 sNpcID);
	///// TEST /////
	...
	void TempleEventLeave();

	// Handles new quest packets
	void QuestV2PacketProcess(Packet & pkt);
	void QuestV2MonsterDataRequest(uint16 uQuestID);
	...
	void ReqAllCharInfoN(Packet & pkt);
	...
	uint16 m_aKillCounts[ACHIEVE_MOB_GROUPS];
...
public:
	...
	DECLARE_LUA_GETTER(isBeginnerKurPor)
	...
	DECLARE_LUA_GETTER(isNoviceKurPor)
	...
	DECLARE_LUA_GETTER(isMasteredKurPor)
	...
	DECLARE_LUA_GETTER(GetCountEmptySlot)
...
/*
	DECLARE_LUA_FUNCTION(RecvDeletedItems) {
		LUA_NO_RETURN(LUA_GET_INSTANCE()->RecvOpenDeletedItems(Packet & pkt));
	}*/
	...
	DECLARE_LUA_FUNCTION(TempleEventLeave) {
		LUA_NO_RETURN(LUA_GET_INSTANCE()->TempleEventLeave());	
	}
