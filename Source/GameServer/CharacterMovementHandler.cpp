...
void CUser::MoveProcess(Packet & pkt)
{
	...
	uint16 will_x, will_z, will_y, new_x, new_z, new_y;
	int16 speed = 0;
	...
	//yenisi
	//06 uint16 oldx uint16 oldy uint16 oldz uint16 speedtype uint8 speed uint16 newx uint16 newy uint16 newz
	pkt >> will_x >> will_z >> will_y >> speed >> echo >> new_x >> new_z >> new_y;
	real_x = new_x/10.0f; real_z = new_z/10.0f; real_y = new_y/10.0f;
	
	//speed 0 ise yürütme . kontrol koy ilk adýmý ile son adýmý arasýndaki farký hesapla 7 birimden yüksekse geri at çarý
	//bura olmamýþ
	if (speed != 0)
		m_sSpeed = speed;
	...
	if (speed == 0)
	{
		if (will_x != new_x || will_y != new_y || will_z != new_z)
			return; // burda eðer gideceði kordinat ile önceki kordinat farklý ile ve hýzý 0 ise returnu çaktýk :D kimsede yoktur bu uskoda bile yok:D
	}
	...
	result << GetSocketID() << will_x << will_z << will_y << speed << echo << new_x << new_z << new_y;
	SendToRegion(&result);
...
void CUser::UserInOut(uint8 bType)
{
	...
	// Edited by Anderson (Room Event View Bug Fix) Start

	if (g_pMain->pTempleEvent.ActiveEvent != -1 && isInTempleEventZone())
	{
		SendToRegion(&result, this, GetEventRoom()); //m_bEventRoom || GetEventRoom()
	}
	else
		SendToRegion(&result);

	// Edited by Anderson (Room Event View Bug Fix) End
...
void CUser::GetUserInfo(Packet & pkt)
{
	...
	pkt << GetRebLevel() << GetCoverTitle() << GetReturnStatus() << uint16(0) << uint8(0) << uint8(1);
}
...
void CUser::ZoneChange(uint16 sNewZone, float x, float z)
{
	...
	if (sNewZone == ZONE_DELOS && pKnightSiege->sMasterKnights == GetClanID())//Ýmza DüslerinSokaðý Kale Sahibi Claný dýþ town atma Fix!
	{
		if (GetNation() == KARUS)
		{
	...
	else if (sNewZone != ZONE_CHAOS_DUNGEON && GetZoneID() == ZONE_CHAOS_DUNGEON)
	{
		...
		sNewZone != ZONE_UNDER_THE_CASTLE && GetZoneID() == ZONE_UNDER_THE_CASTLE)
	{
		SetMaxHp(1);
		g_pMain->UpdateEventUser(this, 0);
	}
	...
	else if ((sNewZone != ZONE_LOST_TEMPLE && GetZoneID() == ZONE_LOST_TEMPLE) || (sNewZone != ZONE_KROWAZ_DOMINION && GetZoneID() == ZONE_KROWAZ_DOMINION)
...
void CUser::CheckWaiting(uint8 sNewZone, uint16 Time)
{
	...
	Packet Error(WIZ_EVENT);
		Error << uint8(1); // Inventory Hatas?.
		Send(&Error);
...
void CUser::RecvZoneChange(Packet & pkt)
{
	...
	else if (opcode == ZoneChangeMilitary)
	{
		uint8 rZone = pkt.read<uint8>();
		//TRACE("Opecode =%d", rZone);

		ZoneChange(rZone, m_curx, m_curz);
	}
